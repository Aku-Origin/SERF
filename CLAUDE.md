...
| Paradoxe du client | **Pas de solution parfaite. On nomme la frontière** : la cryptographie rend sans confiance tout ce qui suit le départ du bulletin, **jamais l'appareil lui-même** | C'est une frontière, pas un trou. On la réduit — build reproductible, démarrage vérifié et reverrouillé, signature multipartite, transparence des binaires, attestation publiée jamais pondérée — et on l'annonce. Sur SERF elle est petite et auditable ; ailleurs elle est large, et c'est la raison technique pour laquelle **le vote suppose SERF** |
...
| ~~Périmètres~~ | **RENVERSÉ le 8 août : pas d'application sur les systèmes des autres.** Voter suppose SERF. À la place : un **installateur en un clic**, sur le modèle de celui de GrapheneOS | Diego : *« ça fait 2 entités à construire, on n'a même pas commencé la première »* — plus le fait qu'une application hors dépôt officiel n'offre aucune garantie de sécurité sur un système qu'on ne contrôle pas. **Prix assumé** : plus de *« la Régence recrute, l'OS convertit »*, et l'entrée par communautés suppose désormais de flasher les appareils. **Vigilance : l'émulation Android existera, l'émulation iOS non** — ce n'est ni techniquement ni juridiquement praticable |
| Vendre des appareils préinstallés | **Reporté** (Diego, 8 août) — pas maintenant | Ce serait la meilleure voie d'adoption (c'est nous qui inscrivons la clé et reverrouillons), mais Diego : *« l'Europe et la France va vouloir arrêter tout ça, ça leur donne le levier, ou alors faut devenir illégal et c'est pas mieux »*. **Doctrine à retenir : la forme de distribution détermine la surface d'attaque.** Un dépôt miroité est difficile à tuer ; une société avec du stock et une garantie légale est facile à tuer |
| Contrôle sans organe permanent | **Jurys tirés au sort et éphémères** : saisis d'une seule question, dissous après · **10 % des Régents hors Table Ronde, plancher 7, plafond 200** · ils **publient, ils n'annulent jamais** | Diego voulait deux organes de contrôle et a trouvé l'objection lui-même : *« on perd le trustless et la confiance par la même occasion »*. La ligne : **la vérification n'a besoin d'aucun organe** (c'est des mathématiques, chacun les refait) ; **le jugement est irréductiblement non-trustless** — il rejoint le paradoxe du client et la coercition parmi les limites qu'on publie. Le plafond existe parce que la représentativité d'un tirage dépend de la taille de l'échantillon, pas du pourcentage : 200 personnes décrivent un million aussi bien que dix mille. *Mandat des jurys : non défini* |
